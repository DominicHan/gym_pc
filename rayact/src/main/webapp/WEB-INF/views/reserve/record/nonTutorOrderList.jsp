                ...
                <form id="searchForm" action="${ctx}/reserve/saleVenue/list"
                    ...
                    <div class="row breadcrumb form-search col-lg-12 col-sm-12"
                        ...
                        <div class="form-group col-lg-2 col-sm-2">
                            ...
                            <select id="venue" name="reserveVenue.id" class="select2 ">
                                <option value="">---请选择---</option>
                                <c:forEach items="${venueList}" var="venue">
                                    <option
                                            <j:if test="${venue.id eq query.reserveVenue.id}">selected="selected"</j:if>
                            ...
                            <table>
                                ...
                                <c:forEach items="${page.list}" var="log">
                                    <tr style="height: 30px;">
                                        ...
                                        <td>${log.collectPrice}</td>
                                        <c:set var="consPriceSum" value="${consPriceSum+log.collectPrice}"></c:set>
                                        <td>${fns:getPayType(log.payType)}</td>
                                        <td>${log.createBy.name}</td>
                                        <td>${log.updateBy.name}</td>
                                        <td><fmt:formatDate value="${log.orderDate}"
                                                            type="date"></fmt:formatDate></td>
                                        <td><fmt:formatDate value="${log.updateDate}"
                                                            type="both"></fmt:formatDate></td>
                                       <j:if test="${userType==1 or userType==5}">
                                            <td>
                                                <a class="btn btn-danger btn-xs"
                                                   href="${ctx}/reserve/saleVenue/delete?orderId=${log.id}"
                                                   onclick="return confirmb('预订人：${log.userName}<br>时间区间：${log.startTime}—${log.endTime}<br>实收金额：${log.collectPrice}<br><br>确认要删除该记录吗？', this.href)">
